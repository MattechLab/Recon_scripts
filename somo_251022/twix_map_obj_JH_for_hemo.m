classdef twix_map_obj_JH_for_hemo < handle
    ...
    properties(GetAccess='public', SetAccess='public')
        ...
        dataType
        rampSampTrj
        flagSelfNav % bastien milani, just for convenience when using the twix_map_obj.
        fidNav
    end
    ...
    methods
        % Constructor:
        function this = twix_map_obj_JH_for_hemo(arg,dataType,fname,version,rstraj)
            ...
            this.fidNav = arg.fidNav; %yiwei send the value to the variable
        ...
        function this = readMDH(this, mdh, filePos )
            ...
            if strcmp(this.dataType,'image')
                %             high_ind = squeeze(double(sLC(:,1))); % JH: Remove all singleton dimensions
                %             high_ind = high_ind(end); % Extract the number of lines exceeding 16 bits
                %
                %             %disp('High index: ')
                %             %disp(high_ind)
                %
                %             this.Lin = double(-(2^16-1):(high_ind+1));
                %
                %             %disp(sLC(:,1));
                %
                %             %this.Lin        = sLC(:,1).' ;
                
                this.Lin = double(sLC(:,1)).';  % JY
                if this.NAcq > 2^16             % JY
                    this.Lin(2^16+1:end) = this.Lin(2^16+1:end) + 2^16; % JY
                end
        ...
        function this = tryAndFixLastMdh(this)
            ...
            while ~isLastAcqGood  &&  this.NAcq > 0  && cnt < 100
                ...
                try
                    this.clean(this.fidNav); %Yiwei added fidNav flag 
        ...
        % Yiwei: add fidNav flag here to toggle the NCol selection
            ...
            %this.NLin = max(this.Lin); %JH
            this.NLin = numel(this.Lin);
            ...
            % Yiwei: this assumption does not work for hemo case.
            % Yiwei added this.fidNav
            if this.fidNav 
...
%                     end
    ...
    methods (Access='protected')
        ...
        function [ixToRaw, ixToTarget] = calcIndices(this)
            ...
            %LinIx = LinIx + (2^16-1); %JH
            % JY    LinIx = LinIx + (2^16); %JH
            % JY    disp(LinIx);
