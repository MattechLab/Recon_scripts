function myTraj = yjTraj_fullRadial3_phyllotaxis_swapXy(varargin)
...
% Authors:
%   Bastien Milani
%   Yiwei Jia sanity check 25.08.26
%   CHUV and UNIL
...
x_0 = reshape(R.*cos(phi).*sin(theta), [1, N_n, nSeg, nShot]); 
y_0 = reshape(R.*sin(phi).*sin(theta), [1, N_n, nSeg, nShot]);
z = reshape(R.*cos(theta),           [1, N_n, nSeg, nShot]);

x = -y_0;
y = -x_0;

% YJ make those inverse...
