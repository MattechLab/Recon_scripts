...
x{3}=x3;
% x{4}=x4;
x{5} = flip(flip(flip(x5,1),2),3);
%%

x1_trans = norm_image(x{3});
x1_coronal = norm_image(rot90(permute(x1_trans, [1,3,2]), 1));
x1_sag = norm_image(flip(permute(x1_trans, [3,2,1]),1));
bmImage(x1_trans);
% bmImage(x1_sag);
% bmImage(x1_coronal);
%
x2_trans = norm_image(x{5});
x2_coronal = norm_image(rot90(permute(x2_trans, [1,3,2]),1));
x2_sag = norm_image(flip(permute(x2_trans, [3,2,1]),1));
bmImage(x2_trans);
% bmImage(x2_sag);
% bmImage(x2_coronal);
%
