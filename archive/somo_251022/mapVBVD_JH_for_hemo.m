function twix_obj = mapVBVD_JH_for_hemo(filename,varargin)
...
while k <= numel(varargin)
    ...
    switch lower(varargin{k})
        ...
        case{'fidnav', 'fidnavigator'} %Yiwei: added arg.fidNav into the function
            if numel(varargin) > k && ~ischar(varargin{k+1})
                arg.fidNav = logical(varargin{k+1});
                k = k+2;
            else
                arg.fidNav = false;
                k = k+1;
...
for s=1:NScans
    ...
    rstraj = [];
    if arg.bReadHeader
        [twix_obj{s}.hdr, rstraj] = read_twix_hdr_JH_for_monalisa(fid);
    end

    % declare data objects:
    twix_obj{s}.image         = twix_map_obj_JH_for_hemo(arg,'image',filename,version,rstraj); %Yiwei added fidNav at the end
    twix_obj{s}.noise         = twix_map_obj_JH_for_hemo(arg,'noise',filename,version);
    twix_obj{s}.phasecor      = twix_map_obj_JH_for_hemo(arg,'phasecor',filename,version,rstraj);
    twix_obj{s}.phasestab     = twix_map_obj_JH_for_hemo(arg,'phasestab',filename,version,rstraj);
    twix_obj{s}.phasestabRef0 = twix_map_obj_JH_for_hemo(arg,'phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.phasestabRef1 = twix_map_obj_JH_for_hemo(arg,'phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.refscan       = twix_map_obj_JH_for_hemo(arg,'refscan',filename,version,rstraj);
    twix_obj{s}.refscanPC     = twix_map_obj_JH_for_hemo(arg,'refscan_phasecor',filename,version,rstraj);
    twix_obj{s}.refscanPS     = twix_map_obj_JH_for_hemo(arg,'refscan_phasestab',filename,version,rstraj);
    twix_obj{s}.refscanPSRef0 = twix_map_obj_JH_for_hemo(arg,'refscan_phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.refscanPSRef1 = twix_map_obj_JH_for_hemo(arg,'refscan_phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.RTfeedback    = twix_map_obj_JH_for_hemo(arg,'rtfeedback',filename,version,rstraj);
    twix_obj{s}.vop           = twix_map_obj_JH_for_hemo(arg,'vop',filename,version); % tx-array rf pulses
    
    % print reader version information
    if s==1
        fprintf('Reader version: %d \n', twix_obj{s}.image.readerVersion); %JH: Just \n
        ...
        if isOctave
            ...
            date_str = datestr(date_str); %JH: windows compability
        end
        fprintf(' (UTC: %s)\n', date_str); 
    end
