    ...
    for lk = 1:m_lProjectionsPerFrame % segments
        for lFrame = 1:m_lNumberOfFrames % shots
            ...
            if flagSelf && lk == 1
                ...
                % m_adPolarAngle(linter) = kost * sqrt(count); % original
                % m_adPolarAngle(linter) = kost * sqrt(count - ((m_lProjectionsPerFrame * m_lNumberOfFrames) * (lpc-1)) ); % similar update as in IDEA, this counter shouldnt update for the polar angle                
                m_adPolarAngle(linter) = kost * sqrt(count - (N * (lpc-1)) ); % similar update as in IDEA, this counter shouldnt update for the polar angle      
